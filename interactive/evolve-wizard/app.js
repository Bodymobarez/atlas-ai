(function () {
  const questions = window.ATLAS_EVOLVE_QUESTIONS || [];
  const STORAGE_KEY = "atlas-evolve-wizard-v1";
  ...
  function buildExport() {
    const lines = [];
    lines.push("# Atlas AI — Evolve Wizard Answers");
    lines.push("");
    ...
    if (lock?.optionId === "A") {
      lines.push("EVOLVE_LOCK: اعتمد خطة التطوير");
    } else if (lock?.optionId === "B") {
      lines.push("EVOLVE_LOCK: راجع");
    } else if (lock?.optionId === "C") {
      lines.push("EVOLVE_LOCK: مسودة");
    }
